...
export default function GettingStarted({ dict }: { dict: GettingStartedDict }) {
	const steps = hardcodedSteps.map((hs, i) => ({
		...hs,
		title: dict.steps[i].title,
		description: dict.steps[i].description,
	}));
		...
		<section className="py-20 bg-gradient-to-br from-brand-500 via-brand-600 to-brand-700 text-white relative overflow-hidden">
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
				<div className="text-center mb-16">
					<span className="inline-block bg-white/20 text-white text-sm font-bold px-4 py-2 rounded-full mb-4 backdrop-blur-sm">{dict.badge}</span>
					<h2 className="text-3xl md:text-5xl font-bold mb-6">{dict.title}</h2>
					<p className="text-lg text-white/80 max-w-2xl mx-auto">{dict.subtitle}</p>
				</div>

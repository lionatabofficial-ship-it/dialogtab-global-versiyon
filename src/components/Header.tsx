	...
	const solutionsItems = [
		{
			...
			desc: "WhatsApp, Instagram, Facebook & more in one place",
			href: "#features",
		},
		{
			icon: (
			...
			title: "DialogTab AI",
			desc: "AI-powered chatbot for automated customer support",
			href: "#features",
		},
		{
			icon: (
			...
			desc: "Sell products directly through WhatsApp conversations",
			href: "https://dialogtab.com/livechat-for-ecommerce",
		},
		{
			icon: (
			...
			desc: "Assign conversations to teams and agents",
			href: "#features",
		},
		{
			icon: (
			...
			desc: "Track performance and customer satisfaction",
			href: "#features",
		},
		{
			icon: (
			...
			title: "Integrations",
			desc: "Connect Shopify, WooCommerce, Stripe & more",
			href: "#integrations",
		},
	];
	...
	const sectorsItems = [
		{
			...
			desc: "Boost online sales with WhatsApp",
		},
		{
			icon: (
			...
			desc: "Manage bookings and inquiries",
		},
		{
			icon: (
			...
			desc: "Reservations & guest communication",
		},
		{
			icon: (
			...
			title: "Automotive",
			desc: "Test drive bookings & service updates",
		},
		{
			icon: (
			...
			desc: "Billing support & service notifications",
		},
		{
			icon: (
			...
			desc: "Secure customer communication",
		},
	];
			...
			{mobileMenuOpen && (
				<div className="lg:hidden bg-white border-t px-6 py-4 space-y-1 shadow-lg">
					...
					<MobileAccordion title="Sectors">
						{sectorsItems.map((item) => (
							<a key={item.title} href="#industries" className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-slate-50" onClick={() => setMobileMenuOpen(false)}>
								<span className="text-brand-600">{item.icon}</span>
								<div>
									<div className="text-sm font-medium text-slate-900">{item.title}</div>
									<div className="text-xs text-slate-500">{item.desc}</div>
					...
					<a href="https://dialogtab.com/livechat-for-ecommerce" className="block text-base font-medium text-slate-700 py-2.5 px-3 rounded-lg hover:bg-slate-50">Ecommerce</a>
					<Link href="#pricing" className="block text-base font-medium text-slate-700 py-2.5 px-3 rounded-lg hover:bg-slate-50" onClick={() => setMobileMenuOpen(false)}>Pricing</Link>
					<Link href="/contact" className="block text-base font-medium text-slate-700 py-2.5 px-3 rounded-lg hover:bg-slate-50" onClick={() => setMobileMenuOpen(false)}>Contact</Link>
					<Link href="/blog" className="block text-base font-medium text-slate-700 py-2.5 px-3 rounded-lg hover:bg-slate-50" onClick={() => setMobileMenuOpen(false)}>Blog</Link>

					<div className="pt-4 border-t space-y-3">
						<a href="https://app.dialogtab.com/" className="block text-center text-base font-medium text-slate-700 py-2.5 border border-slate-300 rounded-full">Login</a>
						<a href="https://app.dialogtab.com/register" className="block w-full text-center rounded-full bg-brand-600 px-5 py-2.5 text-sm font-semibold text-white hover:bg-brand-700 transition-colors">Try for free</a>
					</div>
				</div>
			)}
		</nav>
